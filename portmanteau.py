        ...
        for w2 in words:
            if w1 and w2 and w1 != w2:
                for i in range(1, len(w2) - 1):
                    if w1.endswith(w2[:i]) and i < len(w1):
                        mid = w2[:i]
                        end = w2[i:]
                        start = w1[:len(w1) - len(mid)]
                        t = (start, mid, end)
                        S, M, E = map(len, t)
                        L = S + M + E
                        score = L - abs(S - L / 4) - abs(M - L / 2) - abs(E - L / 4)
                        results.add((t, score))
                        break
    return ''.join(sorted(results, key=lambda x: x[1])[-1][0]) if results else None
