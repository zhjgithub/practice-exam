...
for c, t in mul_score.items():
    for i in base_scores:
        score_names[i * t] = c+str(i)
all_scores = [0] + sorted(score_names, reverse=True)
...
def outcome(target, miss):
    ...
    def neighbor_section(section):
        i = sections.index(section)
        return sections[i - 1], sections[(i + 1) % 20]

    def section_probability(ring, section, probability):
        neighbors = neighbor_section(section)
        return [(ring + section, probability * (1 - miss)), (ring + neighbors[0], probability * miss / 2), (ring + neighbors[1], probability * miss / 2)]
    ...
    if ring == 'T':
        return dict(section_probability(ring, section, 1 - miss) + section_probability('S', section, miss))
    elif ring == 'D':
        return dict([('OFF', miss / 2)] + section_probability('S', section, miss / 2) + section_probability(ring, section, 1 - miss))
    elif ring == 'S':
        return dict(section_probability(ring, section, 1 - miss / 5) + section_probability('T', section, miss / 10) + section_probability('D', section, miss / 10))
...
def best_target(miss):
    ...
    return max(targets, key=lambda x: expected_value(x, miss))
