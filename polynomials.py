...
def poly(coefs):
    ...
    def _f(x):
        return sum(coef * x**p for p, coef in enumerate(coefs))

    _f.coefs = coefs
    size = len(coefs)
    _f.__name__ = ' + '.join(format_term(coef, size - i - 1) for i, coef in enumerate(reversed(coefs)) if coef)
    return _f
...
def format_term(coefficient, power):
    ...
    str_coefficient = format_coefficient(coefficient)
    str_power = format_power(power)
    return '{} * {}'.format(str_coefficient, str_power) if str_coefficient and str_power else str_coefficient or str_power or '1'
...
def add(p1, p2):
    ...
    return poly(tuple(a + b for a,b in itertools.zip_longest(p1.coefs, p2.coefs, fillvalue=0)))


def sub(p1, p2):
    "Return a new polynomial which is the difference of polynomials p1 and p2."
    return add(p1, poly(tuple(map(operator.neg, p2.coefs))))


def mul(p1, p2):
    "Return a new polynomial which is the product of polynomials p1 and p2."
    coefs = collections.defaultdict(int)
    for i, a in enumerate(p1.coefs):
        for j, b in enumerate(p2.coefs):
            coefs[i + j] += a * b
    return poly(tuple(v for k,v in sorted(coefs.items())))
...
def deriv(p):
    ...
    return poly(tuple(c * i for i, c in enumerate(p.coefs) if i != 0))


def integral(p, C=0):
    "Return the integral of a function p (with respect to its argument)."
    return poly((C, ) + tuple(c / (i + 1) for i, c in enumerate(p.coefs)))
